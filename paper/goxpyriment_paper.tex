...
vice-versa). MATLAB can also quite expensive if you one obtain
...
stimuli, expeiment lists, and this have no runtime dependencies. Its
...
\texttt{goxpyriment} achieves equivalent low-level control through the
native SDL3 API \parencite{sdl3}. \texttt{screen.Update()} calls
SDL's \texttt{SDL\_RenderPresent}, which blocks until the display's
vertical retrace when VSYNC is enabled. Variable refresh rate (VRR) monitors
are also supported, as described in the following section.

\subsubsection{Display Compositor Bypass}\label{compositor-bypass}

A compositing window manager (compositor) intercepts every application frame
and blends it with other on-screen elements before sending the result to the
display. This introduces additional latency and jitter that are problematic for
millisecond-accurate stimulus presentation. The degree to which
\texttt{goxpyriment} can avoid this overhead depends on the operating system.

On \textbf{Linux with X11}, SDL3 automatically sets the
\texttt{\_NET\_WM\_BYPASS\_COMPOSITOR} window property when the application
enters fullscreen mode. Compliant compositors (KWin, Mutter, Compton) respond
by \emph{unredirecting} the fullscreen window---routing its framebuffer directly
to the display scan-out pipeline without compositing. The result is presentation
latency below 1~ms, comparable to a compositor-free session.
On \textbf{Linux with Wayland}, the compositor is always the intermediary for
frame delivery, but modern compositors (KWin~5.21+, GNOME Mutter 44+) support
\emph{direct scan-out} for fullscreen applications, achieving similar low-latency
behavior in practice. The best achievable timing on Linux is obtained by running
without any display server: setting the environment variable
\texttt{SDL\_VIDEO\_DRIVER=kmsdrm} before launching the experiment directs SDL3
to communicate with the Linux KMS/DRM subsystem directly, bypassing X11 and
Wayland entirely. This configuration---typically used from a virtual terminal
(\texttt{Ctrl+Alt+F2})---is recommended for the most demanding timing
requirements such as single-frame subliminal stimulation or high-frequency RSVP.

On \textbf{Windows}, SDL3's fullscreen mode creates an exclusive fullscreen
...
\textbf{macOS} is the exception. Apple's Metal rendering pipeline always
delivers frames to the display through the WindowServer compositor, and no
third-party application can bypass it---even in fullscreen. The WindowServer
typically adds one frame of fixed latency (8--17~ms depending on refresh rate)
and 2--5~ms of frame-interval jitter. This onset uncertainty — larger than many
SOAs of experimental interest — makes macOS unsuitable for production data
collection. Researchers should prefer Linux or Windows hardware for laboratory
deployments of \texttt{goxpyriment}; macOS is adequate for pilot testing
and paradigm development.

These platform differences can be quantified empirically with the
\texttt{display} and \texttt{frames} sub-tests of the bundled
\texttt{Timing-Tests} suite.

\subsubsection{Variable Refresh Rate (VRR) Stimulus Presentation}\label{vrr}

Standard displays refresh at a fixed rate (e.g., 60~Hz), which quantises
every stimulus duration to multiples of the frame period (16.67~ms at 60~Hz).
A researcher wishing to present a stimulus for 10~ms, 17~ms, or 23~ms cannot
do so on such a display without relying on temporal dithering or hardware
frame-blending techniques that introduce their own artefacts.
Variable Refresh Rate (VRR) technology---marketed as AMD FreeSync, NVIDIA
G-Sync, or the VESA Adaptive-Sync standard---removes this constraint by
allowing the display to hold each frame for an arbitrary duration and refresh
only when instructed to do so by the GPU.

\texttt{goxpyriment} exposes VRR through a single API call:
\texttt{exp.Screen.SetVSync(0)} disables the VSYNC block, so that every
subsequent \texttt{SDL\_RenderPresent} call returns immediately.
The following pattern then achieves arbitrary stimulus durations with
duration error typically below 0.5~ms (one standard deviation):

\begin{enumerate}
  \item Draw the stimulus and call \texttt{screen.FlipNS()}, which presents
        the frame and records \texttt{onsetNS} (an SDL nanosecond timestamp)
        immediately after \texttt{Present()} returns.
  \item Busy-wait for the desired duration \(d\): spin until
        \(\texttt{time.Now()} \geq \texttt{onset} + d\).
        The last \(\sim500\,\mu s\) use a CPU spin loop (already
        used in the trigger timing routines) to eliminate OS scheduling
        jitter.
  \item Draw the blank screen, call \texttt{FlipNS()} again to record
        \texttt{offsetNS}.
\end{enumerate}

The actual software-controlled duration is
\((\texttt{offsetNS}-\texttt{onsetNS})/10^6\,ms\).  On a VRR-capable
display this equals the true on-screen duration plus a small, constant
hardware pipeline latency (GPU scan-out and panel response, typically
1--5~ms) that can be calibrated once with a photodiode using the
\texttt{frames} timing sub-test of the \texttt{Timing-Tests} suite.

Every VRR panel supports VRR only within a finite refresh-rate window
(e.g., 48--144~Hz, corresponding to frame durations of 6.9--20.8~ms for a
typical gaming monitor). Durations outside this window cause the panel to
revert to fixed-rate behaviour, re-introducing quantisation. The
\texttt{Timing-Tests} suite includes a dedicated \texttt{vrr} sub-test that
sweeps target durations from 1~ms to a user-specified maximum in 1~ms steps
and reports duration errors at each step, directly revealing the panel's VRR
window in the output data.

On a display without VRR support, disabling VSYNC produces frame tearing.
The \texttt{vrr} sub-test detects this automatically: on a non-VRR display the
duration errors cluster at multiples of the frame period rather than being
uniformly small, providing an unambiguous diagnostic.
...
VSYNC sync & Yes (SDL3) & Yes (Pygame/SDL2) & Yes (Pyglet/PTB) & Limited (rAF) \\
VRR / arbitrary durations & Yes (\texttt{SetVSync(0)}) & No & No & No \\
